...
export function removeBySelector(doc: Document, debug: boolean, removeExact: boolean = true, removePartial: boolean = true, mainContent?: Element | null, debugRemovals?: DebugRemoval[], skipHiddenExactSelectors: boolean = false) {
	...
	if (removeExact) {
		...
		exactElements.forEach(el => {
			if (el?.parentNode) {
				...
				// Include div.code for coqdoc-style blocks that haven't been
				// converted to <pre> by standardize yet.
				if (el.closest('pre, code, div.code')) {
					return;
				}
	...
	if (removePartial) {
		...
		allElements.forEach(el => {
			...
			// Include div.code for coqdoc-style blocks not yet converted to <pre>.
			const tag = el.tagName;
			if (tag === 'CODE' || tag === 'PRE' || el.querySelector('pre') || el.closest('code, pre, div.code')) {
				return;
			}
