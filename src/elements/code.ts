...
export const codeBlockRules = [
	{
		selector: [
			...
			// Coqdoc (Rocq/Coq documentation) code blocks
			'div.code'
		].join(', '),
		element: 'pre',
		transform: (el: Element, doc: Document): Element => {
			...
			// Coqdoc (and similar) blocks use semantic markup for syntax
			// highlighting (<span class="id" title="keyword">). Preserve
			// the inner HTML instead of extracting plain text. Unwrap <a>
			// tags first since they're self-referencing anchors, not links.
			if (el.matches('div.code') && el.querySelector('span.id')) {
				el.querySelectorAll('a').forEach(a => {
					a.replaceWith(...Array.from(a.childNodes));
				});
				code.innerHTML = el.innerHTML.replace(/^\s+/, '').replace(/\s+$/, '');
			} else {
