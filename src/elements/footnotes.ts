...
class FootnoteHandler {
	...
	standardizeFootnotes(element: any) {
		...
		// Find the footnote heading anchor BEFORE removing lists, so we can
		// insert the standardized section in the right place.
		const FOOTNOTE_HEADING_RE = /^(?:references|notes|citations|footnotes|bibliography|endnotes)$/i;
		let footnoteInsertAnchor: Element | null = null;
		for (const heading of Array.from(element.querySelectorAll('h2, h3, h4, h5, h6')) as Element[]) {
			if (!FOOTNOTE_HEADING_RE.test(heading.textContent?.trim() || '')) continue;
			// Walk up from the heading — it may be inside a wrapper div (e.g. Wikipedia mw-heading)
			let anchor: any = heading;
			while (anchor.parentElement && anchor.parentElement !== element) {
				anchor = anchor.parentElement;
			}
			footnoteInsertAnchor = anchor;
		}
		...
		// Remove elements orphaned by footnote list removal between the footnote heading
		// and the next heading (empty wrappers, style tags, etc.).
		// Track the last surviving sibling so we insert the new list after it.
		let footnoteInsertAfter: Node | null = footnoteInsertAnchor;
		if (footnoteInsertAnchor) {
			const toRemove: Node[] = [];
			let sib = footnoteInsertAnchor.nextSibling;
			while (sib) {
				if (sib.nodeType === 1) {
					const el = sib as Element;
					if (el.matches?.('h2, h3, h4, h5, h6') || el.querySelector?.('h2, h3, h4, h5, h6')) break;
					if (el.matches?.('style, script') || !(el.textContent || '').trim()) {
						toRemove.push(sib);
					} else {
						footnoteInsertAfter = sib; // meaningful content — insert after it
					}
				}
				sib = sib.nextSibling;
			}
		...
		if (orderedList.children.length > 0) {
			...
			if (footnoteInsertAfter && footnoteInsertAfter.parentNode) {
				// Insert after the heading and any surviving content
				footnoteInsertAfter.parentNode.insertBefore(newList, footnoteInsertAfter.nextSibling);
			} else {
