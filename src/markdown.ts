...
export function createMarkdownContent(content: string, url: string, options?: { macros?: Record<string, string | [string, number]> }) {
	// Page-specific macros for expanding inside prooftree blocks.
	// Passed explicitly or extracted from embedded data-math-macros div.
	let pageMacros: Record<string, string | [string, number]> | null = options?.macros || null;
	if (!pageMacros) {
		const macroMatch = content.match(/data-math-macros='([^']*)'/)
			|| content.match(/data-math-macros="((?:[^"\\]|&quot;)*)"/);
		if (macroMatch) {
			try {
				const raw = macroMatch[1].replace(/&quot;/g, '"').replace(/&apos;/g, "'").replace(/&amp;/g, '&');
				pageMacros = JSON.parse(raw);
			} catch {}
		}
	}

	// Convert remaining raw \[...\] and \(...\) that weren't wrapped by
	// wrapRawLatexDelimiters (e.g. cross-element spans). Only replace
	// delimiters in TEXT content, not inside HTML attribute values.
	// Split on HTML tags to protect attributes.
	content = content.replace(/((?:<[^>]*>)|(?:[^<]+))/g, (segment) => {
		if (segment.startsWith('<')) return segment; // HTML tag — skip
		// First convert \[...\] to $$...$$
		segment = segment.replace(/\\\[([\s\S]*?)\\\]/g, (_m, latex) => `\n$$\n${latex.trim()}\n$$\n`);
		// Strip \(...\) inside prooftree blocks (already in math mode —
		// wrapping in $...$ would create broken nested delimiters).
		segment = segment.replace(/\\begin\{prooftree\}[\s\S]*?\\end\{prooftree\}/g, (block) =>
			block.replace(/\\\(([\s\S]*?)\\\)/g, '$1')
		);
		// Convert remaining \(...\) to $...$
		segment = segment.replace(/\\\(([\s\S]*?)\\\)/g, (_m, latex) => '$' + latex.trim() + '$');
		return segment;
	});
	...
	// MathML <math> elements (from MathJax, etc.) — convert to LaTeX delimiters.
	turndownService.addRule('mathml', {
		filter: (node) => {
			return isGenericElement(node) && node.nodeName.toLowerCase() === 'math';
		},
		replacement: (_content, node) => {
			if (!isGenericElement(node)) return '';
			let latex = (node.getAttribute('data-latex') || node.textContent || '').trim();
			if (!latex) return '';
			// Strip inner math delimiters — the content is already in math
			// mode so nested $...$ or \(...\) would break the wrapper.
			if (latex.includes('$')) {
				// Convert non-C bussproofs \Axiom$...$  →  \Axiom{...}
				// before generic $ stripping.
			...
			// Prooftrees: force display mode and expand custom macros
			// inline (bussproofs parses arguments internally and may not
			// see macros defined in a separate preamble block).
			const hasProoftree = latex.includes('\\begin{prooftree}');
			if (hasProoftree && pageMacros) {
				latex = applyMacroExpansion(latex, pageMacros);
			}
			if (hasProoftree || node.getAttribute('display') === 'block') {
				return `\n$$\n${latex}\n$$\n`;
			}
			return '$' + latex + '$';
		}
	...
	// Math placeholders from MathJax cleanup (<code data-math-latex="...">) —
	// these are created by the clipper when MathJax scripts are converted to
	// inert elements before Defuddle processes the page.
	turndownService.addRule('mathPlaceholder', {
		filter: (node) => {
			return isGenericElement(node) &&
		...
		replacement: (_content, node) => {
			if (!isGenericElement(node)) return '';
			const latex = (node.getAttribute('data-math-latex') || '').trim();
			if (!latex) return '';
			if (node.getAttribute('data-math-display') === 'block') {
				return `\n$$\n${latex}\n$$\n`;
			}
			return '$' + latex + '$';
		}
...
export function toMarkdown(
	...
	const mdOpts = (options as any).macros ? { macros: (options as any).macros } : undefined;
	if (options.markdown) {
		result.content = createMarkdownContent(result.content, url, mdOpts);
	} else if (options.separateMarkdown) {
		result.contentMarkdown = createMarkdownContent(result.content, url, mdOpts);
	}
}

/**
 * Convert bussproofs \begin{prooftree}...\end{prooftree} environments
 * to \cfrac-based LaTeX that renders in standard MathJax/KaTeX (no
 * bussproofs extension needed).
 *
 * Bussproofs is stack-based:
 *   \AxiomC{X}           → push X
 *   \UnaryInfC{X}        → pop 1, push cfrac{popped}{X}
 *   \BinaryInfC{X}       → pop 2, push cfrac{a \quad b}{X}
 *   \TrinaryInfC{X}      → pop 3, push cfrac{a \quad b \quad c}{X}
 *   \noLine               → next inference has no line (use \atop)
 *   \RightLabel{X}        → label on right of next rule
 *   \LeftLabel{X}         → label on left of next rule
 *   \Axiom$a \fCenter b$  → push "a \fCenter b" (sequent syntax)
 *   \UnaryInf$a$          → pop 1, infer
 *   \BinaryInf$a$         → pop 2, infer
 */
function convertProoftreeToCfrac(latex: string): string {
	...
	return latex.replace(
		...
		(_match, body: string) => {
			...
			// Tokenize: match bussproofs commands with their arguments
			const cmdRe = /\\(AxiomC|UnaryInfC|BinaryInfC|TrinaryInfC|RightLabel|LeftLabel|noLine|Axiom|UnaryInf|BinaryInf|def\\fCenter)\s*(?:\{((?:[^{}]|\{[^{}]*\})*)\}|\$([^$]*)\$)?/g;

			let cmd: RegExpExecArray | null;
			while ((cmd = cmdRe.exec(body)) !== null) {
				const name = cmd[1];
				const arg = (cmd[2] ?? cmd[3] ?? '').trim();
				...
				if (name === 'def\\fCenter') {
					continue; // skip \def\fCenter definitions
				}
				if (name === 'AxiomC' || name === 'Axiom') {
					stack.push(arg);
					continue;
				}

				// Inference commands: pop premises, build fraction
				let arity = 1;
				if (name === 'BinaryInfC' || name === 'BinaryInf') arity = 2;
				if (name === 'TrinaryInfC') arity = 3;

				const premises: string[] = [];
				for (let i = 0; i < arity && stack.length > 0; i++) {
					premises.unshift(stack.pop()!);
				}

				const numerator = premises.join(' \\quad ');
				let label = '';
				if (rightLabel) { label = '\\;' + rightLabel; rightLabel = ''; }
				if (leftLabel) { label = leftLabel + '\\;' + label; leftLabel = ''; }

				let result: string;
				if (noLine) {
					// No inference line — just stack vertically
					result = `{\\displaystyle\\frac{\\strut ${numerator}}{\\strut ${arg}}}`;
					// Use a phantom rule to remove the fraction bar
					result = `\\genfrac{}{}{0pt}{0}{${numerator}}{${arg}}`;
					noLine = false;
				} else {
					result = `\\cfrac{${numerator}}{${arg}}`;
				}
			...
			return stack.length > 0 ? stack.join(' \\quad ') : body;
		}
	);
}

/**
 * Expand custom macros (from data-math-macros) inline within LaTeX content.
 * Needed for bussproofs environments where \AxiomC{} arguments are parsed
 * internally and may not see macros defined in a separate preamble block.
 */
function applyMacroExpansion(latex: string, macros: Record<string, string | [string, number]>): string {
	const entries = Object.entries(macros).sort((a, b) => b[0].length - a[0].length);
	for (const [name, value] of entries) {
		const cmd = name.startsWith('\\') ? name : `\\${name}`;
		let expansion = Array.isArray(value) ? value[0] : value;
		const argc = Array.isArray(value) ? value[1] : 0;
		if (argc === 0) {
			// Simplify expansions that bussproofs can't handle:
			// \text{X} → {X}, \mathbf{X} → {X}, etc.
			// Wrap in braces to prevent command merging (e.g. \supsetB)
			expansion = expansion
				.replace(/\\text\{([^}]*)\}/g, '{$1}')
			...
			latex = latex.replace(re, expansion);
		}
	}
	return latex;
}

function expandMacrosInLatex(latex: string, node: TurndownService.Node): string {
	// Find macros from data-math-macros attribute — either on the document
	// element or on a hidden div embedded in the article HTML.
	...
	const macroAttr = doc?.documentElement?.getAttribute?.('data-math-macros')
		|| doc?.querySelector?.('[data-math-macros]')?.getAttribute?.('data-math-macros');
	if (!macroAttr) return latex;

	let macros: Record<string, string | [string, number]>;
	try { macros = JSON.parse(macroAttr); } catch { return latex; }

	// Sort by name length descending so \calV is replaced before \cal
	const entries = Object.entries(macros).sort((a, b) => b[0].length - a[0].length);

	for (const [name, value] of entries) {
		const cmd = name.startsWith('\\') ? name : `\\${name}`;
		const expansion = Array.isArray(value) ? value[0] : value;
		const argc = Array.isArray(value) ? value[1] : 0;

		if (argc === 0) {
			// Simple macro: \rA → \text{A}
			// Match \rA followed by non-letter (word boundary for TeX commands)
			const re = new RegExp(cmd.replace(/\\/g, '\\\\') + '(?![a-zA-Z])', 'g');
			latex = latex.replace(re, expansion);
		}
		// Skip macros with arguments — they're too complex to expand textually
	}
