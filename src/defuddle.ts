...
export class Defuddle {
	...
	private parseInternal(overrideOptions: Partial<DefuddleOptions> = {}): DefuddleResponse {
		...
		try {
			...
			// Convert custom code playground elements to standard <pre><code>
			// before removals, since their code lives in HTML comments (no text
			// content) and the heuristic scorer would discard them.
			profileStep('convertCodePlaygrounds', () => this.convertCodePlaygrounds(clone));
	...
	/**
	 * Convert custom code playground elements (e.g. dev.java's <java-playground>)
	 * to standard <pre><code> blocks. These elements store source code inside HTML
	 * comments within <snippet> children, so they have no text content and would be
	 * discarded by the heuristic scorer.
	 */
	private convertCodePlaygrounds(doc: Document): void {
		const playgrounds = doc.querySelectorAll('java-playground');
		if (playgrounds.length === 0) return;

		let converted = 0;
		for (const pg of playgrounds) {
			const snippet = pg.querySelector('snippet');
			if (!snippet) continue;

			// Extract code from comment nodes inside <snippet>
			let code = '';
			for (const node of Array.from(snippet.childNodes)) {
				if (node.nodeType === 8) { // Comment node
					code += (node as Comment).data;
				}
			}
			...
			codeEl.setAttribute('class', 'language-java');
			codeEl.setAttribute('data-lang', 'java');
			codeEl.textContent = code;
			pre.appendChild(codeEl);
			pg.parentNode?.replaceChild(pre, pg);
			converted++;
		}

		if (converted > 0) {
			this._log('Converted code playgrounds:', converted);
		}
	}
