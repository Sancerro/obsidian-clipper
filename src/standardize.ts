...
function stripUnwantedAttributes(element: Element, debug: boolean): void {
	...
	const processElement = (el: Element) => {
		...
		const isInCodeBlock = !!el.closest('pre, code');
...
function standardizeElements(element: Element, doc: Document, subProfile?: Record<string, number>): void {
	...
	// Collapse IPA tooltip spans: Wikipedia wraps each phonetic character
	// in <span title="...">x</span>. Without collapsing, turndown inserts
	// whitespace between adjacent inline spans, producing "/ m ə ˈ r uː n /"
	// instead of "/məˈruːn/".
	stepSE('collapseIPASpans', () => {
		const containers = element.querySelectorAll('.IPA');
		for (const container of Array.from(containers)) {
			const spans = container.querySelectorAll('span[title]');
			for (const span of Array.from(spans)) {
				if (!span.parentNode) continue;
				// Only unwrap if the span contains just text (no child elements)
